...
std::vector<unsigned int> compileShader(const ShaderDesc& desc)
{
    ...
    const char* sourceBytes = desc.source.c_str();
    shader.setStrings(&sourceBytes, 1);
    shader.setEntryPoint("main");
    //shader.setPreamble(preamble.c_str());

    DirStackFileIncluder includer;
    includer.pushExternalLocalDirectory(desc.includeDir);
    ...
    glslang::GlslangToSpv(*program.getIntermediate(glslType), byteCode, &options);
